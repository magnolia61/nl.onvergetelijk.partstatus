...
function leeftijd_civicrm_customPre(string $op, int $groupID, int $entityID, array &$params): void {
    ...
    // Alleen draaien voor profiel 'Deelname Intern' (ID 271)
...
function leeftijd_civicrm_criteria($array_partditevent, $leeftijd_ditevent_decimalen)
{
    ...
    $groepklas  = $array_partditevent['part_groepklas'] 	?? NULL;
    ...
    if ($kampkort == 'kk1' OR $kampkort == 'kk2') {
        if (in_array($groepklas, array("klas_3"))) { $new_groepklas = 'groep_3'; }
        if (in_array($groepklas, array("klas_4"))) { $new_groepklas = 'groep_4'; }
        if (in_array($groepklas, array("klas_5"))) { $new_groepklas = 'groep_5'; }
        if (in_array($groepklas, array("klas_6"))) { $new_groepklas = 'groep_6'; }
    }
    if ($kampkort == 'tk1' OR $kampkort == 'tk2') {
        if (in_array($groepklas, array("groep_2"))) { $new_groepklas = 'klas_2'; }
        if (in_array($groepklas, array("groep_3"))) { $new_groepklas = 'klas_3'; }
    }
    if ($kampkort == 'jk1' OR $kampkort == 'jk2') {
        if (in_array($groepklas, array("groep_3"))) { $new_groepklas = 'klas_3'; }
        if (in_array($groepklas, array("groep_4"))) { $new_groepklas = 'klas_4'; }
        if (in_array($groepklas, array("groep_5"))) { $new_groepklas = 'klas_5'; }
        if (in_array($groepklas, array("groep_6"))) { $new_groepklas = 'klas_6'; }
    }
    ...
    if ($kampkort == 'kk1' AND in_array($groepklas, array("groep_3","groep_4","groep_5","groep_6","groep_7"))) { $criteria_school = 'prima'; }
    if ($kampkort == 'kk2' AND in_array($groepklas, array("groep_3","groep_4","groep_5","groep_6","groep_7"))) { $criteria_school = 'prima'; }
    if ($kampkort == 'bk1' AND in_array($groepklas, array("groep_8","klas_1")))                                { $criteria_school = 'prima'; }
    if ($kampkort == 'bk2' AND in_array($groepklas, array("groep_8","klas_1")))                                { $criteria_school = 'prima'; }
    if ($kampkort == 'tk1' AND in_array($groepklas, array("klas_2","klas_3")))                                 { $criteria_school = 'prima'; }
    if ($kampkort == 'tk2' AND in_array($groepklas, array("klas_2","klas_3")))                                 { $criteria_school = 'prima'; }
    if ($kampkort == 'jk1' AND in_array($groepklas, array("klas_4","klas_5","klas_6","vervolg")))              { $criteria_school = 'prima'; }
    if ($kampkort == 'jk2' AND in_array($groepklas, array("klas_4","klas_5","klas_6","vervolg")))              { $criteria_school = 'prima'; }

    // Marge Check School
    if (in_array($kampkort,array("tk1","tk2")) AND in_array($groepklas,array("klas_4")))                       { $criteria_school = 'marge'; }
    if (in_array($kampkort,array("jk1","jk2")) AND in_array($groepklas,array("klas_2","klas_3")))              { $criteria_school = 'marge'; }
    ...
    if ($contact_id) {
        ...
        if ($new_groepklas) $params_contact['values']['DITJAAR.ditjaar_groep_klas'] = $new_groepklas;
...
function leeftijd_civicrm_status($array_partditevent, $array_criteria = NULL) {
    ...
    // Criteria ophalen (uit meegegeven array of bestaande data)
    $ditevent_criteria_oordeel  = $array_criteria['criteria_oordeel'] 	?? $array_partditevent['part_criteria_oordeel'] 	?? NULL;
    $ditevent_criteria_indicatie= $array_criteria['criteria_indicatie'] ?? $array_partditevent['part_criteria_indicatie'] 	?? NULL;
    ...
    $new_wachtlijst_erop 		= NULL;
    $new_wachtlijst_eraf 		= NULL;
    $new_criteriacheck_start 	= NULL;

    // --- STATUS LOGICA ---

    // Correctie van onbekende status (0, 5, 6) naar geregistreerd of wacht op goedkeuring
    if (in_array($ditevent_part_status_id, [0, 5, 6]) && $ditevent_part_rol == 'deelnemer' && empty($ditevent_wachtlijst_erop)) {
        if ($ditevent_criteria_indicatie == 'criteriaprima') {
            $new_status_id = 1; // Registered
            $new_status_label = 'Bevestigd';
        } elseif ($ditevent_criteria_indicatie == 'criteriawijktaf' && $ditevent_criteria_oordeel != 'buitencriteria') {
    ...
    if ($ditevent_part_status_id == 1) $new_status_label = 'Bevestigd';
    
    // Status Wachtlijst (7)
    if ($ditevent_part_status_id == 7) {
        $new_status_label = 'Wachtlijst';
        $new_wachtlijst_erop = $ditevent_register_date; // Forceer datum
    }
    ...
    // Criteriacheck Start Datum Logica
    if ($ditevent_criteria_oordeel == 'oordeelnietnodig') {
        $new_criteriacheck_start = "";
    }
    if ($ditevent_criteria_oordeel == 'oordeelnognodig' || $new_status_id == 8) {
        // Als nog niet gezet, zet nu
        if (empty($ditevent_criteriacheck_start)) {
            $new_criteriacheck_start = $ditevent_register_date;
        }
    }
    ...
    // --- UPDATE UITVOEREN ---
    $updates = [];
    if ($new_status_id != $ditevent_part_status_id) $updates['status_id'] 							 = $new_status_id;
    if ($new_status_label) 							$updates['PART.deelnamestatus:label'] 			 = $new_status_label;
    if ($new_wachtlijst_erop) 						$updates['PART_DEEL_INTERN.wachtlijst_erop'] 	 = $new_wachtlijst_erop;
    if ($new_wachtlijst_eraf) 						$updates['PART_DEEL_INTERN.wachtlijst_eraf'] 	 = $new_wachtlijst_eraf;
    if ($new_criteriacheck_start !== NULL) 			$updates['PART_DEEL_INTERN.criteriacheck_start'] = $new_criteriacheck_start;

    if ($ditevent_part_id && !empty($updates)) {
        $params_part = [
            'checkPermissions' => FALSE,
            'where' => [['id', '=', $ditevent_part_id]],
            'values' => $updates,
        ...
        wachthond($extdebug, 2, "STATUS UPDATED", "$new_status_id ($new_status_label)");
    }
